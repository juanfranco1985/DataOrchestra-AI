...
from dataorchestra.cli import run_data_quality, run_preflight, run_readiness
...
def test_readiness_blocks_when_data_quality_is_below_target(tmp_path: Path):
    repo_root = tmp_path / "repo"
    write_required_docs(repo_root)
    workspace = create_client_workspace(tmp_path / "runtime" / "clients", "cliente_quality_block")
    client_dir = Path(workspace["client_dir"])
    raw_dir = client_dir / "raw"
    (raw_dir / "ventas.csv").write_text(
        "fecha,producto,categoria,cantidad,precio_unitario,costo_unitario\n"
    ...
    assert run_preflight(client_dir)["status"] == "ready_for_analysis"
    assert run_data_quality(client_dir)["can_support_diagnostic"] is False

    result = run_readiness(client_dir, repo_root=repo_root)

    assert result["can_continue"] is False
    assert "data_quality_below_target" in check_ids(result)
